...
/**
 * Remove top-level MDX `import` / `export` (ESM) statements, which are always
 * at column 0 and outside code fences. Lines inside fenced code blocks are
 * preserved, so code samples that contain import/export are left intact.
 */
function stripMdxEsm(raw: string): string {
  let inFence = false
  return raw
    .split("\n")
    .filter((line) => {
      ...
      if (!inFence && /^(import|export)\s/.test(line)) {
        return false
      }
      return true
    })
